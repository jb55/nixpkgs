...

{
  ...
  options.programs.gnupg = {
    ...
    trezor-agent.package = mkOption {
      type = types.package;
      default = pkgs.python3Packages.trezor_agent;
      description = "The trezor_agent package to use";
    };
  ...
  config = mkIf cfg.agent.enable {
    ...
    systemd.user.services.trezor-agent = {
      ...
      serviceConfig.ExecStart = "${cfg.trezor-agent.package}/bin/trezor-gpg-agent";
    };
  });
